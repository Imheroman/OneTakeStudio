...
1. **Core Service** (코어 서비스) - MySQL (core_db)
2. **Media Service** (미디어 서비스) - MySQL (media_db) ← PostgreSQL에서 통합 마이그레이션됨
...
**데이터베이스 특성**:
- **MySQL (core_db)**: 트랜잭션 일관성이 중요한 비즈니스 데이터
- **MySQL (media_db)**: 미디어 메타데이터, 세션 정보, 분석 데이터 (단일 DB 인프라로 통합)
